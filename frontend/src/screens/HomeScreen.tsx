        ...
        {recurring.status === 'ready' && (
          <div className="recurring-card">
            <div className="recurring-card-row">
              ...
              <span className="tag tag-neutral" style={{ marginLeft: 'auto' }}>{recurring.data.intervalLabel}</span>
            </div>
            <div>
              <div className="recurring-title">{recurring.data.itemIds.length} itens · {recurring.data.totalWeightKg} kg</div>
              <div className="text-muted recurring-summary">{recurring.data.summaryLabel}</div>
            </div>
            <div className="recurring-card-row" style={{ alignItems: 'baseline' }}>
              <span className="recurring-price">{money(recurring.data.contractTotal)}</span>
            ...
            <div className="recurring-actions">
              <button
                ...
                style={{ flex: 1 }}
                onClick={async () => { await cart.repeatRecurring(recurring.data.id); go({ screen: 'cart' }); }}
              >
